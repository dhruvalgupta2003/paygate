...
### Does Limen hold my funds?

No. You specify a receiving wallet; settlements go directly from agent
to your wallet. Limen only sees the transaction metadata it needs to
verify.

### Does Limen take a cut?

The open-source SDK and proxy take **zero fees**. The hosted service at
`limen.dev` charges 1.5% on processed volume for managed analytics,
...
`X-PAYMENT` header can use Limen. We publish client SDKs for
...
identifiable person and a DSR arrives, run `limen dsr redact --wallet
...
Limen marks the transaction `upstream_failed` and emits a
...
Limen waits for `confirmations_base` blocks (default 2) before
...
unmodified alongside Limen; ask us before using the logo in a product
you're building.
