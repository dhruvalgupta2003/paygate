...
## Symptom
`rate(limen_rpc_failures_total[5m]) > 0.05` for 3+ minutes, **or** all
...
## Diagnosis
1. Dashboard → `limen-rpc`: which provider is red?
2. `limen_rpc_failures_total{provider}` — is one provider offline, or
   all?
3. `curl -sS $LIMEN_BASE_RPC_URL -X POST -d '{"jsonrpc":"2.0","method":"eth_blockNumber","id":1}'`
   from a proxy pod.
...
- If both providers are down: add a temporary third via env-var
  injection: `kubectl set env deployment/limen-proxy
  LIMEN_BASE_RPC_URL=...`.
...
- Are we hitting rate limits? Check
  `limen_rpc_failures_total{status="429"}`.
- Did a config change remove a provider?
- Did traffic spike (limen_requests_total) without capacity planning?
