/**
 * `limen demo` — drive a full x402 handshake against a running proxy.
 *
 ...
 *
 * Intended for local verification against `limen start --dev`.
...
export async function runDemo(opts: DemoOptions): Promise<void> {
  ...
  if (opts.submit) {
    ...
    const rpc = opts.rpcUrl ?? process.env['LIMEN_BASE_SEPOLIA_RPC_URL'] ?? 'https://sepolia.base.org';
  ...
  // 202 means "settlement pending" — Limen accepted the authorisation but
