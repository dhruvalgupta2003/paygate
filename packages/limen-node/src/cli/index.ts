...
import { LIMEN_VERSION } from '../constants.js';
...
program
  .name('limen')
  .description('Limen CLI — x402 paywall for agent traffic.')
  .version(LIMEN_VERSION);

program
  .command('start')
  .description('Run the proxy.')
  .option('-c, --config <path>', 'path to limen.config.yml', './limen.config.yml')
  .option('-u, --upstream <url>', 'upstream URL to proxy to', process.env['LIMEN_UPSTREAM_URL'])
...
program
  ...
  .option('--rpc-url <url>', 'RPC URL for --submit (default: $LIMEN_BASE_SEPOLIA_RPC_URL or sepolia.base.org)')
...
program.parseAsync(process.argv).catch((err) => {
  console.error(`limen: ${(err as Error).message}`);
  process.exit(1);
});
