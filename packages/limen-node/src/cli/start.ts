...
export async function startServer(opts: StartOptions): Promise<void> {
  ...
  const upstream = opts.upstream ?? process.env['LIMEN_UPSTREAM_URL'];
  if (!upstream) {
    throw new Error('upstream URL is required (--upstream or LIMEN_UPSTREAM_URL)');
  }

  const redisUrl = process.env['LIMEN_REDIS_URL'];
  ...
  const compliance = cfg.compliance.sanctions_screening
    ? new DefaultComplianceScreen({
        circleApiKey: process.env['LIMEN_CIRCLE_API_KEY'] ?? undefined,
  ...
  const facilitator =
    cfg.defaults.facilitator === 'coinbase' && opts.dev !== true
      ? new FacilitatorClient({
          ...
          ...(process.env['LIMEN_FACILITATOR_API_KEY']
            ? { apiKey: process.env['LIMEN_FACILITATOR_API_KEY'] }
            : {}),
        })
      : undefined;
  ...
  if (cfg.wallets.base) {
    adapters['base'] = new BaseAdapter({
      chainId: 'base',
      rpcUrl: process.env['LIMEN_BASE_RPC_URL'] ?? 'https://mainnet.base.org',
  ...
  if (cfg.wallets.solana) {
    adapters['solana'] = new SolanaAdapter({
      chainId: 'solana',
      rpcUrl: process.env['LIMEN_SOLANA_RPC_URL'] ?? 'https://api.mainnet-beta.solana.com',
  ...
  app.all('*', async (c) => {
    ...
    return c.body(
      (result.response.body ?? null) as never,
  ...
  server.listen(port, opts.host, () => {
    logger.info({ host: opts.host, port, upstream }, 'limen listening');
  });
