...
export type LimenConfig = z.infer<typeof configSchema>;
export type EndpointConfig = z.infer<typeof endpointSchema>;

export function loadConfigFromString(raw: string): LimenConfig {
  const parsed = raw.trim().startsWith('{') ? JSON.parse(raw) : parseYaml(raw);
  return parseConfig(parsed);
}

export function loadConfigFromFile(path: string): LimenConfig {
  const raw = readFileSync(path, 'utf-8');
  return loadConfigFromString(raw);
}

export function parseConfig(input: unknown): LimenConfig {
