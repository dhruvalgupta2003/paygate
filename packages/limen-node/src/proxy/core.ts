...
import type { LimenConfig } from '../config.js';
...
const tracer = trace.getTracer('limen');

export interface CoreProxyDeps {
  readonly config: LimenConfig;
...
export interface CoreProxyResult {
  readonly response: LimenResponse;
...
export class CoreProxy {
  ...
  private readonly cfg: LimenConfig;
  ...
  async handle(request: LimenRequest): Promise<CoreProxyResult> {
    ...
    return tracer.startActiveSpan(
      'limen.handle',
      { attributes: { 'limen.path': request.path, 'limen.method': request.method } },
      async (span: Span) => {
        try {
          ...
          try {
            ...
            if (err instanceof LimenError) return { response: this.errorResponse(err) };
            throw err;
          }
          ...
          return {
            response: this.errorResponse(
              new LimenError({ code: 'INTERNAL', detail: 'unexpected error', cause: err }),
            ),
          };
        } finally {
          span.end();
   ...
   *   LIMEN_API_URL           e.g. http://localhost:4020
   *   LIMEN_API_INGEST_TOKEN  shared bearer token
  ...
  private async reportSettlement(info: {
    ...
    const apiUrl = process.env['LIMEN_API_URL'];
    const token = process.env['LIMEN_API_INGEST_TOKEN'];
    if (!apiUrl || !token) return;

    const slug = this.cfg.project?.slug ?? 'demo-api';
    const res = await fetch(`${apiUrl.replace(/\/$/, '')}/_limen/v1/transactions/ingest`, {
      method: 'POST',
      headers: {
  ...
  private errorResponse(err: LimenError): LimenResponse {
  ...
  private async forward(request: LimenRequest): Promise<LimenResponse> {
    ...
    try {
      ...
      if ((err as Error).name === 'AbortError') {
        throw new LimenError({ code: 'UPSTREAM_TIMEOUT', detail: `upstream exceeded ${this.cfg.advanced.upstream_timeout_ms}ms` });
      }
      throw new LimenError({ code: 'UPSTREAM_FAILED', detail: (err as Error).message, cause: err });
    } finally {
...
function headersToObject(h: LimenRequest['headers']): Record<string, string> {
