...
def lint(path: str = typer.Option("./limen.config.yml", "-c", "--config")) -> None:
    load_config(path)
    typer.echo(f"{path}: OK")


@config_app.command("print")
def print_cmd(path: str = typer.Option("./limen.config.yml", "-c", "--config")) -> None:
    cfg = load_config(path)
    typer.echo(json.dumps(cfg.model_dump(mode="json"), indent=2))


@config_app.command("explain")
def explain(path: str = typer.Option("./limen.config.yml", "-c", "--config")) -> None:
    cfg = load_config(path)
...
def migrate(path: str = typer.Option("./limen.config.yml", "-c", "--config")) -> None:
