"""`limen doctor` — check config + connectivity."""
...
from limen.constants import DEFAULT_FACILITATOR_URL
...
def run(
    ...
    if cfg is not None:
        if cfg.wallets.base:
            ok, detail = asyncio.run(_rpc_ok(os.environ.get("LIMEN_BASE_RPC_URL", "https://mainnet.base.org"), "evm"))
            results.append(("base rpc", ok, detail))
        if cfg.wallets.solana:
            ok, detail = asyncio.run(_rpc_ok(os.environ.get("LIMEN_SOLANA_RPC_URL", "https://api.mainnet-beta.solana.com"), "solana"))
