"""`limen verify` — verify an on-chain transaction."""
...
from limen.utils.amount import usdc_to_micros
...
def _verify_evm(chain: str, tx: str, expected_amount: str | None, expected_to: str | None) -> None:
    ...
    rpc = (
        os.environ.get("LIMEN_BASE_RPC_URL", "https://mainnet.base.org")
        if chain == "base"
        else os.environ.get("LIMEN_BASE_SEPOLIA_RPC_URL", "https://sepolia.base.org")
    )
...
def _verify_solana(
    ...
    rpc = (
        os.environ.get("LIMEN_SOLANA_RPC_URL", "https://api.mainnet-beta.solana.com")
        if chain == "solana"
        else os.environ.get("LIMEN_SOLANA_DEVNET_RPC_URL", "https://api.devnet.solana.com")
    )
    client = Client(rpc)
