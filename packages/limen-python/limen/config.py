"""Config schema + loader. Mirrors ``packages/limen-node/src/config.ts``.
...
class LimenConfig(_SchemaModel):
    """Top-level Limen configuration. Load via :func:`parse_config`."""
...
def parse_config(data: Any) -> LimenConfig:
    """Validate a raw mapping against the Limen schema.

    Raises :class:`LimenError` (BAD_CONFIG) on validation errors so the
    caller sees the stable error surface.
    """
    try:
        return LimenConfig.model_validate(data)
...
def load_config_from_string(raw: str) -> LimenConfig:
    stripped = raw.lstrip()
...
def load_config_from_file(path: str | Path) -> LimenConfig:
    p = Path(path)
    try:
...
def load_config(path: str | Path) -> LimenConfig:
