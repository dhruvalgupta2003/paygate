...
Mirror of ``packages/limen-node/src/errors.ts`` — codes, HTTP statuses,
...
class LimenError(Exception):
    """Base exception. Every Limen error carries:
    ...
    def to_json(self) -> dict[str, Any]:
        ...
        body["docs"] = f"https://limen.dev/docs/errors#{self.code.value.lower()}"
        body.update(self.extra)
        return body

    def __repr__(self) -> str:
        return f"LimenError(code={self.code.value}, http={self.http}, detail={self.detail!r})"


def is_limen_error(e: object) -> bool:
    return isinstance(e, LimenError)
