...
All real logic lives in :class:`limen.proxy.core.CoreProxy`; this file is
a thin adapter that converts an ASGI request into a ``LimenRequest`` and
...
class LimenMiddleware:
    ...
    async def __call__(
        ...
        # When Limen intercepts (402/429/etc.), return its response and
...
async def _send_response(send: ASGISend, response: LimenResponse) -> None:
