...
Usage::
    ...
    app = Flask(__name__)
    app.wsgi_app = paygate_middleware(app.wsgi_app, wallets={...}, endpoints=[...])
"""
...
def paygate_middleware(app: WSGIApp, **deps: Any) -> WSGIApp:
    """Wrap a WSGI app with PayGate's 402 handshake."""
...
def _send_wsgi(response: PayGateResponse, start_response: WSGIStartResponse) -> list[bytes]:
