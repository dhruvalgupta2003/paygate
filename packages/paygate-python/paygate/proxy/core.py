...
request/response primitives into :class:`PayGateRequest` /
:class:`PayGateResponse` and hand the work to :class:`CoreProxy`.

This mirrors ``packages/paygate-node/src/proxy/core.ts`` step-for-step so
...
from ..config import PayGateConfig
...
class CoreProxy:
    ...
    def __init__(self, deps: CoreProxyDeps) -> None:
        ...
        self._logger = deps.logger or get_logger("paygate.proxy")
    ...
    async def handle(self, request: PayGateRequest) -> CoreProxyResult:
        ...
        try:
            ...
            if not verify.ok:
                ...
                return CoreProxyResult(
                    response=self._error_response(
                        PayGateError(code=fail.code, detail=fail.detail)
                    ),
                    auth=auth,
                )
    ...
    def _error_response(self, err: PayGateError) -> PayGateResponse:
        body = err.to_json()
    ...
    async def _forward(self, request: PayGateRequest) -> PayGateResponse:
