#!/usr/bin/env bash
# Orchestrate a clean Limen demo in one terminal so you can screen-record
...
cleanup() {
  ...
  pkill -f 'limen-demo-upstream' 2>/dev/null || true
}
...
python3 -c "
import sys
sys.argv[0] = 'limen-demo-upstream'
...
# --- start the proxy ------------------------------------------------------
if [[ ! -f packages/limen-node/dist/cli.js ]]; then
  echo "!! CLI not built.  Running: pnpm --filter @limen/node build"
  pnpm --filter @limen/node build
fi

echo ">> starting limen proxy on :$PROXY_PORT (dev mode)"
node packages/limen-node/dist/cli.js start \
